class Api::ScoresController < ApplicationController
  ...
  def level_score
    if score_params[:num].to_i > 1820
        return false
    elsif score_params[:num].to_i >= 1428
      score_params[:level].to_i == 5
    elsif score_params[:num].to_i >= 1050
      score_params[:level].to_i == 4
    elsif score_params[:num].to_i >= 686
      score_params[:level].to_i == 3
    elsif score_params[:num].to_i >= 336
      score_params[:level].to_i == 2
    else
      true
    end
  end
